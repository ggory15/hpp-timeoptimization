...
namespace hpp{
    namespace timeopt {
      ...
      void PlannerSetting::initialize(const std::string& cfg_file, const std::string& default_file, const std::string& planner_vars_yaml)
      {
        cfg_file_ = cfg_file;
        save_dynamics_file_ = cfg_file.substr(0, cfg_file.size()-5) + "_dyn_results.yaml";
        default_solver_setting_file_ = default_file; //CFG_SRC_PATH + std::string("default_solver_setting.yaml");
